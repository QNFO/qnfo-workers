...
var UA = "qnfo-chat-canary/" + VERSION;
...
async function aiFetch(env, path, init) {
  const target = env.QNFO_AI || { fetch: /* @__PURE__ */ __name((u, o) => fetch(u, o), "fetch") };
  return target.fetch(ROUTER + path, init);
}
__name(aiFetch, "aiFetch");
...
__name(isFallback, "isFallback");
async function chatProbe(env, model, prompt, threadId, uaOverride2) {
  const t0 = Date.now();
  const r = await aiFetch(env, "/v1/chat/completions", {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: "Bearer " + env.ROUTER_AUTH_KEY, "User-Agent": uaOverride2 || UA },
...
__name(chatProbe, "chatProbe");
async function record(env, cron, probe, model, ok, detail, latency) {
  try {
    await env.AUDIT.prepare("INSERT INTO chat_canary (ts, cron, probe, model, ok, latency_ms, detail) VALUES (?1,?2,?3,?4,?5,?6,?7)").bind((/* @__PURE__ */ new Date()).toISOString(), cron, probe, String(model || "").slice(0, 60), ok ? 1 : 0, latency || 0, String(detail || "").slice(0, 400)).run();
  } catch (e) {
  }
}
...
async function alert(env, level, message) {
  try {
    const dup = await env.AUDIT.prepare("SELECT COUNT(*) AS n FROM alerts WHERE source='chat-canary' AND date(created_at)=?1 AND message=?2").bind((/* @__PURE__ */ new Date()).toISOString().slice(0, 10), String(message).slice(0, 200)).first();
    if (dup && dup.n) return;
    ...
    if (env.SEND_EMAIL && env.ALERT_EMAIL_TO) {
      try {
        const r = await env.SEND_EMAIL.send({ to: env.ALERT_EMAIL_TO, from: { email: FROM_EMAIL, name: FROM_NAME }, subject: "QNFO chat-canary " + level, text: String(message).slice(0, 2e3) });
...
async function runCanary(env) {
  ...
  if (!(ranToday && ranToday.n)) {
    ...
    if (!ok5) {
      try {
        await env.AUDIT.prepare("DELETE FROM intent_express_log WHERE thread_id = ?1").bind(opsThread).run();
      } catch (e2) {
      }
      try {
        await env.AUDIT.prepare("DELETE FROM intents WHERE desire LIKE ?1").bind("%" + opsPhrase.slice(0, 60) + "%").run();
      } catch (e3) {
      }
...
__name(runSentGuard, "runSentGuard");
var worker_default = {
  async scheduled(event, env, ctx) {
    const cron = event && event.cron || "";
    try {
      if (cron.indexOf("30 6") === 0) {
        const out = await runSentGuard(env);
        console.log("sentguard", JSON.stringify(out));
      } else {
        const out = await runCanary(env);
        console.log("canary", JSON.stringify(out));
      }
    } catch (e) {
      console.error("chat-canary", String(e && e.message || e));
    }
  },
