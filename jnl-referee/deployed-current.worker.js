...
// worker.js
var VERSION = "0.5.1";
var MODELS_DEFAULT = "@cf/openai/gpt-oss-120b,@cf/meta/llama-4-scout-17b-16e-instruct";
...
function decisionFrom(parsedList, basis) {
  ...
  return { decision, avg: Math.round(avg * 100) / 100, fatal, disagreement, speculative, reason: "avg=" + Math.round(avg * 100) / 100 + " min=" + minAvg + " max=" + maxAvg + " fatal=" + fatal + " speculative=" + speculative + " basis=" + basis + " lowconf=" + anyLowConfidence };
}
...
async function runReview(env, recid, manual) {
  ...
  for (var i = 0; i < models.length; i++) {
    try {
      ...
      if (p) {
        ...
        await log(env, recid, "parse", "model " + models[i] + " returned non-JSON len=" + outTxt.length);
      }
    } catch (e) {
  ...
  var p0 = parsedList[0] || {};
  var sound = Math.round(parsedList.map(function(p2) {
    return clampScore(p2.score_soundness);
  }).reduce(function(a, b) {
    return a + b;
  }, 0) / parsedList.length);
  var novel = Math.round(parsedList.map(function(p2) {
    return clampScore(p2.score_novelty);
  }).reduce(function(a, b) {
    return a + b;
  }, 0) / parsedList.length);
  var clar = Math.round(parsedList.map(function(p2) {
    return clampScore(p2.score_clarity);
  }).reduce(function(a, b) {
    return a + b;
  }, 0) / parsedList.length);
  var repro = Math.round(parsedList.map(function(p2) {
    return clampScore(p2.score_reproducibility);
  }).reduce(function(a, b) {
    return a + b;
  }, 0) / parsedList.length);
  var fatalCount = parsedList.map(function(p2) {
    return arrOf(p2.fatal_flaws).length;
  }).reduce(function(a, b) {
    return a + b;
  }, 0);
  await env.AUDIT.prepare(
  ...
  return { ok: true, recid, decision: dec.decision, avg_score: dec.avg, basis, text_chars: rec.text_chars, models: modelsUsed, fatal_flaws: fatalCount, disagreement: dec.disagreement };
}
...
async function handleScheduled(env) {
  try {
    ...
    return { ok: true, seeded, ran, budget };
  } catch (e) {
  ...
  async fetch(request, env) {
    ...
    try {
      if (path === "/health") {
        ...
        return json({ ok: true, service: "jnl-referee", version: VERSION, schema, counts, models: String(env.JNL_MODELS || MODELS_DEFAULT) });
      }
      if (path === "/queue") {
      ...
      if (method === "POST" && (path === "/enqueue" || path === "/seed" || path === "/run" || path === "/run-next" || path === "/cal" || path === "/submit" || path === "/rebuild-graph")) {
        ...
        if (path === "/cal") {
          ...
          var calText = String(bodyC.text || "").slice(0, 16e3);
