...
var AgentSessionProvider = class {
	...
	*/
	...
	applyCompactions(messages, compactions) {
		...
		const messageIndexById = new Map(ids.map((messageId, index) => [messageId, index]));
		const result = [];
		let i = 0;
		while (i < messages.length) {
			const matching = compactions.filter((compaction) => compaction.fromMessageId === ids[i] && (messageIndexById.get(compaction.toMessageId) ?? -1) >= i);
...
var Session = class Session {
	...
	*/
	async compact(leafId) {
		...
		let result;
		try {
			result = await this._compactionFn(history, { tokenCounter: this._tokenCounter });
		} catch (err) {
		...
		const existing = (await this.getCompactions()).filter((compaction) => historyIds.has(`compaction_${compaction.id}`) || historyIds.has(compaction.fromMessageId) && historyIds.has(compaction.toMessageId));
