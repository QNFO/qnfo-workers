...
import { c as browserLinks, d as browserScrape, i as DurableBrowserSessionStore, l as browserMarkdown, o as browserContent, s as browserExtract, t as BrowserConnector } from "../connector-CptFKzRh.js";
import { tool } from "ai";
import { z } from "zod";
...
function browserScreenshotOutput(value) {
	const outer = typeof value === "object" && value !== null ? value : null;
	const result = outer && typeof outer.result === "object" && outer.result !== null ? outer.result : outer;
	if (result?.type !== "browser_screenshot" || typeof result.mediaType !== "string" || typeof result.data !== "string") return null;
	return result;
}
const BASE64_REDACTION_THRESHOLD = 4096;
const MAX_REDACTION_DEPTH = 20;
const MAX_REDACTION_NODES = 1e4;
function base64Details(value, minimumLength = BASE64_REDACTION_THRESHOLD) {
...
function base64Redaction(value, mediaType, minimumLength) {
...
function redactBase64Payloads(value) {
	...
	function visit(current, depth) {
		...
		if (current instanceof ArrayBuffer || ArrayBuffer.isView(current) || current instanceof Date || current instanceof Map || current instanceof Set) return current;
		...
		try {
			...
			const screenshot = record.type === "browser_screenshot" && typeof record.mediaType === "string";
			return Object.fromEntries(Object.entries(record).map(([key, entry]) => [key, screenshot && key === "data" && typeof entry === "string" ? base64Redaction(entry, record.mediaType, 0) : visit(entry, depth + 1)]));
		} finally {
...
function transformBrowserResult(value) {
	return browserScreenshotOutput(value) ? value : truncateResult(redactBase64Payloads(value));
}
function browserExecuteModelOutput(output) {
	...
	try {
		...
		return {
			type: "json",
			value: serialized === void 0 ? null : JSON.parse(serialized)
		};
	} catch {
		return {
			type: "text",
			value: "Browser execution completed, but its result could not be serialized for model context."
		};
	}
}
/**
...
function createBrowserRuntime(options) {
	...
	const tools = { browser_execute: {
		...runtime.tool({ connectorHints: { cdp: isKitesurf ? "Kitesurf one-shot Browser CDP. Call codemode.describe(\"cdp\") for connector types and Kitesurf execution rules. codemode.search indexes connector methods and snippets, not underlying CDP commands. Complete the task in one execution and do not pause. Return screenshots as { type: \"browser_screenshot\", mediaType: \"image/png\", data: screenshot.data }." : "Browser CDP. Return screenshots as { type: 'browser_screenshot', mediaType: 'image/png', data: screenshot.data }; the UI keeps the image while the model receives a compact summary." } }),
		toModelOutput: ({ output }) => browserExecuteModelOutput(output)
	} };
	if (options.quickActions !== false && (!isKitesurf || options.quickActions !== void 0)) {
		const qa = typeof options.quickActions === "object" ? options.quickActions : {};
...
function createQuickActionTools(options) {
	...
	if (enabled.has("extract")) tools.browser_extract = tool({
		...
		inputSchema: z.object({
			...
			schema: z.record(z.string(), z.unknown(), { error: "Schema must be a JSON object" }).optional().describe("Optional JSON Schema describing the desired output")
