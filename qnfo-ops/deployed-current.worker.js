...
async function fleetStatus(env) {
  ...
  for (const f of FLEET) {
    ...
    out.push({ name: f.name, healthy: h.ok, http: h.http, version: h.body && (h.body.version || "") || "", error: h.error || null, count });
  }
  let apiList = [];
  if (env.CF_API_TOKEN) {
    try {
...
async function execTool(env, name, rawArgs, userText) {
  ...
  return { tool_call_id: null, name, ok: !!(res && res.ok), text: text.length > 8e3 ? text.slice(0, 8e3) + "...(truncated)" : text };
}
__name(execTool, "execTool");
async function logToolEvent(env, name, args, res, ms) {
  if (!env.QNFO_AUDIT) return;
  try {
    await env.QNFO_AUDIT.prepare("INSERT INTO cloud_ops_events (id, ts, kind, text, meta, job, status) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)").bind(randId("evt-"), iso(), "ops_ai_tool", name, snippet({ args, resultOk: !!(res && res.ok), ms }, 600), "qnfo-ops", res && res.ok ? "ok" : "error").run();
  } catch (e) {
  }
}
...
async function callDeepSeek(env, messages, maxTokens, withTools) {
  const body = { model: UPSTREAM_MODEL, messages, max_tokens: maxTokens, temperature: 0.5, top_p: 0.9, stream: false };
  if (withTools) {
...
__name(callDeepSeekClient, "callDeepSeekClient");
...
async function handleChat(env, body, authHeader, ua, ctx) {
  ...
  const prompt = lastUserText(msgs).slice(0, 4e3);
  ...
  const allUserText = msgs.filter((m) => m.role === "user").map((m) => String(m.content || "")).join(" ").slice(0, 8e3);
  ...
  if (_externalOnly && !_opsIntent) {
    try {
      ...
      const cText = String(cMsg.content || "");
      const cToolCalls = Array.isArray(cMsg.tool_calls) && cMsg.tool_calls.length ? cMsg.tool_calls : null;
      const cToolCallsIndexed = (cToolCalls || []).map(function(tc0, i0) {
        return Object.assign({}, tc0, { index: tc0 && tc0.index != null ? tc0.index : i0 });
      });
      ...
      if (_outsideCalls.length) {
        ...
        const cRespId = randId("chatcmpl-");
        const cCreated = Math.floor(Date.now() / 1e3);
        const cRec = { id: randId("ops-"), ts: iso(), model: wanted, strategy: "client-tools", prompt, response: (cText || (cToolCalls ? JSON.stringify(cToolCalls) : "")).slice(0, 2e4), prompt_tokens: cUsage.prompt_tokens || estTokens(JSON.stringify(msgs)), completion_tokens: cUsage.completion_tokens || estTokens(cText), cost_usd: 0, latency_ms: Date.now() - t0, tool_calls: cToolCalls ? JSON.stringify(cToolCalls).slice(0, 3e3) : "", source: detectSource(ua), ua: String(ua || "").slice(0, 200), streamed: isStream ? 1 : 0, ok: 1 };
      ...
      ctx.waitUntil(logOps(env, { id: randId("ops-"), ts: iso(), model: wanted, strategy: "client-tools", prompt, response: errText.slice(0, 2e3), latency_ms: Date.now() - t0, tool_calls: "", source: detectSource(ua), ua: String(ua || "").slice(0, 200), streamed: isStream ? 1 : 0, ok: 0 }));
  ...
  const loopDeadline = Date.now() + 2e4;
  try {
    ...
    ctx.waitUntil(logOps(env, { id: randId("ops-"), ts: iso(), model: wanted, strategy: "agent", prompt, response: errText.slice(0, 2e3), latency_ms: Date.now() - t0, tool_calls: JSON.stringify(toolLog).slice(0, 3e3), source: detectSource(ua), ua: String(ua || "").slice(0, 200), streamed: isStream ? 1 : 0, ok: 0 }));
    return json({ error: errText }, 502);
  }
  ...
  const logRec = { id: randId("ops-"), ts: iso(), model: wanted, strategy: toolLog.length ? "agent-tools" : "chat", prompt, response: content.slice(0, 2e4), prompt_tokens: promptTokens, completion_tokens: completionTokens, cost_usd: costUsd, latency_ms: latencyMs, tool_calls: JSON.stringify(toolLog).slice(0, 3e3), source: detectSource(ua), ua: String(ua || "").slice(0, 200), streamed: isStream ? 1 : 0, ok: 1 };
  ctx.waitUntil(logOps(env, logRec));
  if (isStream) {
    const enc = new TextEncoder();
    const nlnl = String.fromCharCode(10, 10);
    const chunk = /* @__PURE__ */ __name(function(delta, finish) {
      return enc.encode("data: " + JSON.stringify({ id: respId, object: "chat.completion.chunk", created, model: wanted, choices: [{ index: 0, delta, finish_reason: finish }] }) + nlnl);
    }, "chunk");
...
async function cfAnalytics(env) {
  ...
  try {
    ...
    for (const row of rows2) {
      ...
      reqs += n;
      const w = row.dimensions && row.dimensions.worker || "unknown";
      byWorker[w] = (byWorker[w] || 0) + n;
    }
    out.worker_invocations_30d = { requests: reqs, by_worker: Object.entries(byWorker).slice(0, 8).map(function(e) {
