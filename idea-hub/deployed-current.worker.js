...
IDEA: `;
  ...
  var m0D = {
    async fetch(request, env, ctx) {
      ...
      if (p === "/idea-factory" || p.startsWith("/idea-factory/")) {
        const u = new URL(request.url);
        u.pathname = p.slice(13) || "/";
        return ideafactoryMod.default.fetch(new Request(u.toString(), request), env, ctx);
      }
      if (p === "/idea-miner" || p.startsWith("/idea-miner/")) {
        const u = new URL(request.url);
        u.pathname = p.slice(11) || "/";
        return ideaminerMod.default.fetch(new Request(u.toString(), request), env, ctx);
      }
      if (p === "/idea-triage" || p.startsWith("/idea-triage/")) {
        const u = new URL(request.url);
        u.pathname = p.slice(12) || "/";
        return ideatriageMod.default.fetch(new Request(u.toString(), request), env, ctx);
      }
      if (p === "/" || p === "") {
        const u = new URL(request.url);
        u.pathname = "/";
      ...
      return ideafactoryMod.default.fetch(request, env, ctx);
    },
    async scheduled(event, env, ctx) {
      const c = event.cron;
...
var worker_default = {
  ...
  async scheduled(event, env, ctx) {
    const c = event.cron;
    if (c === "*/10 * * * *") {
      m0.default.scheduled(event, env, ctx);
      return;
    }
    if (c === "0 * * * *") {
      m0.default.scheduled(event, env, ctx);
