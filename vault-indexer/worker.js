...
async function run(env, cap) {
  ...
  for (var x = 0; x < work.length; x++) {
    var text = texts[x];
    if (text === null) { s.skipped++; continue; }
    var w = work[x];
    var chs = chunkText(text);
    if (chs.length === 0) { s.skipped++; continue; }
    var resp;
    ...
    if (valid.length === 0) { s.errors++; continue; }
    var cat = categorize(chs.join(" "), w.key);
    var dg = await sha256hex(w.path);
    var vecBatch = [], chunkStmts = [];
    for (var ci = 0; ci < valid.length; ci++) {
      var id = dg + ":" + ci;
      vecBatch.push({ id: id, values: valid[ci], metadata: { path: w.path, type: extOf(w.key), chunk: String(ci), category: String(cat), modified: String(w.obj.uploaded ? w.obj.uploaded.toISOString() : ""), text: sanitize(chs[ci], 800) } });
      chunkStmts.push(env.PERSONAL.prepare("INSERT INTO chunks (id,path,chunk_idx,text_len) VALUES (?1,?2,?3,?4) ON CONFLICT(id) DO UPDATE SET path=?2,chunk_idx=?3,text_len=?4").bind(id, w.path, ci, chs[ci].length));
    }
...
async function handle(request, env, ctx) {
  try {
    ...
    if (url.pathname === "/stats" && request.method === "GET") {
      ...
      return json({ ok: true, version: VERSION, files: t && t.c || 0, chunks: ch && ch.c || 0 });
    }
