...
var LW_BASE = "https://www.lesswrong.com";
var EA_FEED = "https://forum.effectivealtruism.org/feed.xml";
var HN_API = "https://hn.algolia.com/api/v1/search";
function nowIso() {
...
function clean(s) {
  return stripHtml(s).slice(0, 400);
}
__name(clean, "clean");
function escQ(s) {
...
async function fetchText(url, timeoutMs) {
  ...
  const t = setTimeout(() => ctl.abort(), timeoutMs || 12e3);
  try {
...
__name(relevanceFor, "relevanceFor");
...
__name(parseLwSearch, "parseLwSearch");
...
__name(parseHn, "parseHn");
async function run(env, forced) {
  const t0 = nowIso();
  const cfg = /* @__PURE__ */ __name(async (key) => {
    const r = await env.RADAR_DB.prepare("SELECT value FROM venue_radar_config WHERE key = ?").bind(key).first();
    return r ? r.value : null;
  }, "cfg");
  const setCfg = /* @__PURE__ */ __name((key, value) => env.RADAR_DB.prepare("INSERT OR REPLACE INTO venue_radar_config (key, value, updated_at) VALUES (?, ?, ?)").bind(key, value, t0).run(), "setCfg");
  ...
  if (enabled !== "1") return { ok: true, worker: WORKER, version: VERSION, skipped: "disabled", ts: t0 };
  {
    ...
    if (last) {
      ...
      if (!Number.isNaN(dt) && dt < 6 * 3600 * 1e3 && !forced) return { ok: true, worker: WORKER, version: VERSION, skipped: "backoff", ts: t0 };
      if (!Number.isNaN(dt) && forced && dt < 5 * 60 * 1e3) return { ok: true, worker: WORKER, version: VERSION, skipped: "too-frequent", ts: t0 };
    }
  }
  ...
  for (const b of BUCKETS) {
    ...
    else rows = parseLwSearch(f.text, q, b);
    let kept = 0;
    for (const r of rows) {
      const ins = await env.RADAR_DB.prepare(INSERT).bind("lesswrong", r.id, r.kind, r.title, r.author, r.url, r.karma, r.date, r.query, r.topic, r.snippet, r.relevance, r.raw_hash, t0).run();
      if (ins && ins.meta && ins.meta.changes > 0) kept++;
    }
    summary.push({ venue: "lesswrong", bucket: b.code, fetched: rows.length, kept, err });
    await env.RADAR_DB.prepare("INSERT INTO venue_radar_runs (venue, kind, run_at, status, fetched, kept, detail) VALUES (?,?,?,?,?,?,?)").bind("lesswrong", b.code, t0, err ? "error" : "ok", rows.length, kept, err || "").run();
  }
  {
    ...
    let kept = 0;
    for (const r of rows) {
      const ins = await env.RADAR_DB.prepare(INSERT).bind("eaforum", r.id, r.kind, r.title, r.author, r.url, r.karma, r.date, r.query, r.topic, r.snippet, r.relevance, r.raw_hash, t0).run();
      if (ins && ins.meta && ins.meta.changes > 0) kept++;
    }
    summary.push({ venue: "eaforum", bucket: "RSS", fetched: rows.length, kept, err });
    await env.RADAR_DB.prepare("INSERT INTO venue_radar_runs (venue, kind, run_at, status, fetched, kept, detail) VALUES (?,?,?,?,?,?,?)").bind("eaforum", "rss", t0, err ? "error" : "ok", rows.length, kept, err || "").run();
  }
  for (const b of BUCKETS) {
    const q = b.label;
    ...
    let rows = [], err = null;
    if (f.error) err = f.error;
    else rows = parseHn(f.text, q, b);
    let kept = 0;
    for (const r of rows) {
      const ins = await env.RADAR_DB.prepare(INSERT).bind("hackernews", r.id, r.kind, r.title, r.author, r.url, r.karma, r.date, r.query, r.topic, r.snippet, r.relevance, r.raw_hash, t0).run();
      if (ins && ins.meta && ins.meta.changes > 0) kept++;
    }
    summary.push({ venue: "hackernews", bucket: b.code, fetched: rows.length, kept, err });
    await env.RADAR_DB.prepare("INSERT INTO venue_radar_runs (venue, kind, run_at, status, fetched, kept, detail) VALUES (?,?,?,?,?,?,?)").bind("hackernews", b.code, t0, err ? "error" : "ok", rows.length, kept, err || "").run();
  }
