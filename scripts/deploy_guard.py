...
Owner id defaults to deepchat/<hostname>-<pid>.
...
def main(argv):
    ...
    if cmd == "acquire":
        ...
        st, j = call("POST", "/lock/acquire", {"worker": w, "owner": OWNER, "ttl_sec": ttl, "expected_version": ev})
        print(json.dumps(j))
        if st == 200:
    ...
    if cmd == "with-lock":
        ...
        runcmd = argv[sep + 1:]
        st, j = call("POST", "/lock/acquire", {"worker": w, "owner": OWNER, "ttl_sec": 1800, "expected_version": None})
        ...
        try:
            ...
            call("POST", "/ledger", {"worker": w, "actor": OWNER, "from": frm, "to": to,
