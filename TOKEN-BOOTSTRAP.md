...
## Artifacts control-plane tokens (2026-09-24)

Two workers, TWO DISTINCT secrets. A single shared value would mean one leak
compromises both gates, so they are deliberately different.

| Worker | Secret name | Local file (never committed) |
|---|---|---|
| qnfo-artifacts | ARTIFACTS_TOKEN | `~/.deepchat/secrets/artifacts-token.txt` |
| qnfo-artifact-agent | ARTIFACTS_TOKEN | `~/.deepchat/secrets/artifacts-agent-token.txt` |

Each worker REJECTS the other's token (verified 2026-09-24), so a stray token
cannot reach the wrong control plane.

### Rotation runbook

Secrets cannot be read back, so a lost local file means re-issue.

1. Generate: `python -c "import secrets;print(secrets.token_urlsafe(32))" > <file>`
2. `printf '%s' "$(cat <file>)" | npx wrangler secret put ARTIFACTS_TOKEN --name <worker>`
3. **Wait ~1 minute.** The `/health` `auth` flag and the gate both follow the new
   value only after propagation. Verifying immediately produces a FALSE FAIL --
   this happened on 2026-09-24 (read as agent+old=200/new=401, i.e. exactly backwards).
4. Verify AFTER the wait:
   - `curl -H "Authorization: Bearer $TOK" .../robots` -> 200
   - same request with the OTHER worker's token -> 401
   - no auth -> 401
   - with the secret deleted -> 503 (fail-closed), never 200
