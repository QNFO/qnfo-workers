...
function slugify(s) { return String(s || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 80) || "paper"; }

async function logEvent(env, kind, text, status) {
  try {
    const id = "re-" + Date.now().toString(36) + "-" + Math.floor(Math.random() * 1e6).toString(36);
    await env.QNFO_AUDIT.prepare("INSERT INTO cloud_ops_events (id, ts, kind, text, meta, job, status) VALUES (?,?,?,?,?,?,?)")
      .bind(id, nowIso(), kind, String(text).slice(0,800), "{}", WORKER, status || "ok").run();
  } catch (e) {}
}
...
async function zenodo(env, method, path, body, attempt) {
  ...
  const n = attempt || 0;
  try {
    const r = await fetch(url, { method: method, headers: { "User-Agent": "QNFO-research-exec/0.4.6", "Content-Type": "application/json" }, body: body ? JSON.stringify(body) : undefined });
    if ((r.status >= 500 || r.status === 429) && n < 4) {
      await new Promise(function(res) { setTimeout(res, 2500 * (n + 1)); });
      return zenodo(env, method, path, body, n + 1);
    }
    const text = await r.text();
    try { return JSON.parse(text); } catch (e) { return { _status: r.status, _text: text.slice(0,200) }; }
  } catch (e) {
    if (n < 4) {
      await new Promise(function(res) { setTimeout(res, 2500 * (n + 1)); });
      return zenodo(env, method, path, body, n + 1);
...
async function publishToZenodo(env, title, abstract, bodyMd, slug) {
  ...
  const meta = await zenodo(env, "PUT", "/" + id, { metadata: {
    ...
    description: (abstract || title).slice(0, 3000),
  ...
  if (bucket) {
    ...
    const up = await fetch(fileUrl, { method: "PUT", headers: { "Content-Type": "text/markdown", "User-Agent": "QNFO-research-exec/0.4.5" }, body: bodyMd });
...
async function genNote(env, row) {
  ...
  if (!note) { await markError(env, row, "note empty (all models)"); return { ok:false, stage:"note" }; }
  await env.QNFO_AUDIT.prepare("UPDATE research_queue SET stage='draft', context=? WHERE id=?").bind(String(note).slice(0,6000), row.id).run();
...
async function genPaper(env, row) {
  ...
  await env.LIVING_PAPER.prepare("INSERT OR REPLACE INTO papers (identifier, title, authors, abstract, body_md, slug, version, status, identifier_type, doi, zenodo_doi, paper_type, license, language) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
    .bind("qnf-" + row.source_id, title, JSON.stringify([AUTHOR]), abstract, String(paper).slice(0,120000), slug, "1.0.0", "review", "qnfo", null, null, "preprint", "CC BY 4.0", "en").run();
  await env.QNFO_AUDIT.prepare("UPDATE research_queue SET stage='publish', status='review', paper_slug=?, context=? WHERE id=?").bind(slug, String(note).slice(0,6000), row.id).run();
  return { ok:true, stage:"draft->publish", slug:slug, title:title };
}
...
async function publishStage(env, row) {
  ...
  try {
    await env.QNFO_AUDIT.prepare("INSERT INTO dissemination_tracker (id, paper_slug, paper_doi, paper_title, channel, action, mode, fallback, zenodo_url, pages_url, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,datetime('now'),datetime('now'))")
      .bind("res-" + Date.now().toString(36), slug, pub.doi, paper.title, "bluesky", "queued", "auto", 0, pub.record, "https://papers.qnfo.org/papers/" + slug + "/").run();
  } catch (e) {}
  ...
  return { ok:true, stage:"publish->published", doi:pub.doi, slug:slug };
}
...
async function run(env) {
  ...
  try {
    ...
    if (row) { const r = await publishStage(env, row); await logEvent(env, "done", JSON.stringify(r).slice(0,500), r.ok ? "ok":"error"); return { status: r.ok?"ok":"error", res:r }; }
    row = await env.QNFO_AUDIT.prepare("SELECT * FROM research_queue WHERE status='researching' AND stage='draft' LIMIT 1").first();
    if (row) { const r = await genPaper(env, row); await logEvent(env, "done", JSON.stringify(r).slice(0,500), r.ok ? "ok":"error"); return { status: r.ok?"ok":"error", res:r }; }
    row = await env.QNFO_AUDIT.prepare("SELECT * FROM research_queue WHERE status='researching' AND stage='note' LIMIT 1").first();
    if (row) { const r = await genNote(env, row); await logEvent(env, "done", JSON.stringify(r).slice(0,500), r.ok ? "ok":"error"); return { status: r.ok?"ok":"error", res:r }; }
