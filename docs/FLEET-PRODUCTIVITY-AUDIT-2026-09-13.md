...
## 5. Cron configuration is not in source control
...
`qnfo-cloud-ops/`, `qnfo-observability/` and `personal-companion/` do have `wrangler.toml`.
...
### 6.5 UNRESOLVED CONTRADICTION — the `alerts` table is a moving target

Two aggregates over `alerts` minutes apart in the same session:

- `GROUP BY level, digested` -> critical 806, warning 186, info 92, error 16, info/NULL 1 (**1101 rows**)
- `GROUP BY digested` -> `1 -> 169` (**169 rows**)

Both are direct tool returns. Either the table is being pruned concurrently, or one read is
wrong. **I cannot reconcile them and I assert neither.** Consequence: no alert count in this
document should be treated as exact. `qnfo-observability`'s own v1.1.5 note documents a
`digested` enum containing `'auto'` (914) which neither of my reads observed.

### 6.6 Deploy healer — kill switch is ENGAGED

`fleet_deploy_state` at 2026-09-13T14:23:10Z: `enabled=0`, `auto_heal=0`. `fleet_deploys`
after 14:20 returns **0 rows** — the hourly retry loop has stopped.

Three targets were in the loop before the switch:

| worker | error | count |
|---|---|---|
| personal-companion | HTTP 400 code 10021 `Workflow GenerationFlow must be exported` | ~12+ consecutive hourly |
| qnfo-cloud-ops | HTTP 400 code 10021 `Uncaught SyntaxError: Invalid or unexpected token at worker.js:1:2` | 25+ consecutive hourly |
| qnfo-observability | HTTP 400 code 10021 `No such module "fleet.js" imported from worker.js` | 1 (id 76, 14:04:01) |
...
**`qnfo-cloud-ops` corruption is in the R2 artifact, not in source.** Both repo copies —
`QNFO/qnfo-workers/qnfo-cloud-ops/worker.js` (sha 59dd468d) and
`QNFO/qnfo-ops/cloud/scheduler/worker.js` (sha 1851450d) — begin with valid
`import { connect } from "cloudflare:sockets";`. The syntax error at `worker.js:1:2` exists
only in `r2:qnfo-canonical/qnfo-cloud-ops.js`. Remediation is a canonical re-sync, not a
source edit.

**`qnfo-observability` fix already exists.** Repo `worker.js` is v1.1.6-single-module, which
inlines `FLEET` and drops `import './fleet.js'`. Canonical R2 is v1.1.4 (multi-module). The
fix is written and undeployed.

### 6.7 `worker_invocations` is a dead probe table emitting false 5xx
...
## 8. Remediation queue — every item is deploy-gated
...
(prefix `canonical` and `qnfo-canonical` both return 0 objects in `audit` and `releases`).
...
2. Re-sync `r2:qnfo-canonical/qnfo-cloud-ops.js` from `QNFO/qnfo-ops/cloud/scheduler/worker.js`.
3. Deploy `qnfo-observability` v1.1.6-single-module (fix already in repo).
4. Reconcile canonical `personal-companion` to >= v1.1.0 **and** export `GenerationFlow`,
   *then* fix `qnfo-fleet-control/version-compare.mjs` (the leading-`v` misparse).
   Do not fix 10021 before this.
5. Fix the `qnfo-pipeline-ops` alert predicates: `idea_proposals` status `new` does not exist;
   add dedupe so a closed ticket is not re-filed hourly.
6. Restore `worker_activity_daily` roster to all deployed workers; purge the 25 ghost rows.
7. Give `worker_invocations` resolvable endpoints or delete the table.
8. Rotate the 9 `credentials/*` objects and restrict `BACKUPS_R2`.
