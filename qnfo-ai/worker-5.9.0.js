...
function classify(prompt) {
  ...
  if (/\b(code|javascript|python|typescript|function|api|bug|debug|compile|sql|regex)\b/.test(p)) {
    ...
    complexity = "high";
    verifiability = "self";
  } else if (/\b(prove|theorem|proof|math|physics|quantum|paper|research|cite|arxiv|jpcub|qwav|paqit|qnfo|joules[- ]per[- ](solution|compute)|energy[- ]metric|energy[- ]standard)\b/.test(p)) {
    domain = "science";
    complexity = "high";
    divergence = "low";
...
async function runEnsemble(env, messages, maxTokens, domain) {
  ...
  const intendedPrimary = useCoderPrimary ? ENSEMBLE.primary.wa : "deepseek-v4-flash";
  ...
  try {
    if (useCoderPrimary) {
      ...
      const primary = await withTimeout(callDeepSeek(env, MODELS["deepseek-v4-flash"].api, messages, maxTokens, false), 40000, "ensemble-primary-deepseek");
      primaryText = extractWAContent(primary);
    }
  } catch (e) {
    primaryText = "";
...
async function handleChat(env, body, authHeader, ctx, ua) {
  ...
  let webSources = null;
  if (body.web || isCurrentEvents(lastUserText(messages))) {
  ...
  if (env.QNFO_INFRA && env.INFRA_TOKEN && !ragOff && (ragForce || cls.domain === "science" || /\b(jpcub|qwav|paqit|qnfo|joules[- ]per[- ](solution|compute))\b/i.test(lastUserText(messages).slice(0, 300)))) {
  ...
  if (isEnsemble || autoEnsemble) {
    try {
      const ensResp = /* @__PURE__ */ __name((content, body2) => {
        const logRec = { ...mkLogRec(), model: "ensemble", streamed: isStream ? 1 : 0, response: String(content).slice(0, 4e3), prompt_tokens: estimateInputTokens(messages), completion_tokens: estimateOutputTokens(content), latency_ms: Date.now() - t0 };
...
if (effSpec.api) {
  ...
  try {
    ...
    if (!content && !toolCalls && effSpec.wa && !wantsCode) {
      const fbSpec = MODELS["qwen2.5-coder-32b"] || MODELS["gemma-4-26b"] || null;
      if (fbSpec && fbSpec.wa !== effSpec.wa) {
        try {
          const fbTurn = await runModelTurn(env, fbSpec, messages, max_tokens, null, 0.3, 0.95);
          if (fbTurn.content && String(fbTurn.content).trim()) { content = fbTurn.content; provider = fbTurn.provider || provider; }
        } catch (e) {}
      }
    }
    ...
    const logRec = { ...mkLogRec(), streamed: 0, response: content.slice(0, 4e3), prompt_tokens: estimateInputTokens(messages), completion_tokens: estimateOutputTokens(content), latency_ms: Date.now() - t0 };
    if (env.QNFO_AUDIT || env.LOG_VZ) ctx.waitUntil(logQuery(env, logRec));
    ...
    if (env.QNFO_AUDIT && record.thread_id) {
      ...
      if (_isPublicRow) {
        await env.QNFO_AUDIT.batch([
          env.QNFO_AUDIT.prepare("INSERT OR REPLACE INTO chat (id, thread, ts, role, content, model, source) VALUES (?1,?2,?3,?4,?5,?6,?7)").bind(record.id + "-u", record.thread_id, record.ts, "user", String(record.prompt || "").slice(0, 8e3), record.model, "qnfo-ai"),
          env.QNFO_AUDIT.prepare("INSERT OR REPLACE INTO chat (id, thread, ts, role, content, model, source) VALUES (?1,?2,?3,?4,?5,?6,?7)").bind(record.id + "-a", record.thread_id, record.ts, "assistant", String(record.response || "").slice(0, 8e3), record.model, "qnfo-ai")
        ]);
      }
    }
  } catch (e) {
...
function streamWithLog(upstream, env, ctx, rec) {
  ...
  ctx.waitUntil(done.then(() => logQuery(env, { ...rec, response: acc.slice(0, 4e3), streamed: 1 })).catch(() => {
  }));
