// ============================================================================
// qnfo-paper-reviser v1.0.0
// Autonomous adversarial audit + revision subloop.
//
// PURPOSE: ensure every QNFO research publication (published + Zenodo-deposited)
// has been adversarially audited and revised at least once (>= 2 versions).
//
// Canonical source: QNFO/qnfo-workers/qnfo-paper-reviser/worker.js
// ============================================================================

const VERSION = "1.0.4";
// MODEL-PER-TASK-1 (2026-09-11, user directive: no llama, no tiny models): was
// llama-3.3-70b-instruct-fp8-fast. Replaced with a frontier model because this worker
// ADVERSARIALLY AUDITS AND REVISES PUBLISHED RESEARCH, where fabricated attribution is
// the highest-cost failure. Measured: llama-3.3-70b invented named living mathematicians
// as doing work absent from the supplied source.
const MODEL = "@cf/deepseek-ai/deepseek-v4-pro-0813";
const BATCH = 3;
const UA = "QNFO-paper-reviser/" + VERSION + " (+https://papers.qnfo.org)";

const PROV_FILES = ["references.bib", "citation-audit.md", "DUE-DILIGENCE.md", "PROJECT-PLAN.md", "README.md", "LICENSE"];
...
async function zenodoGet(path) {
  const r = await fetch("https://zenodo.org/api/records" + path, { headers: { "User-Agent": UA }, signal: AbortSignal.timeout(30000) });
  if (!r.ok) return { _status: r.status };
  try { return await r.json(); } catch (e) { return { _status: r.status, _parseError: e.message }; }
}
...
function parseRefLine(raw) {
  ...
  let arxiv = "", doi = "";
  const ax = rest.match(/arXiv:\s*([^\s]+)/); if (ax) arxiv = ax[1];
  const dm = rest.match(/DOI:\s*([^\s,;]+)/i); if (dm) doi = dm[1];
  return { authors, year, title, rest, arxiv, doi };
}
function refKey(p, i) { const a = (p.authors || "").replace(/[^A-Za-z]/g, "").slice(0, 14) || "ref"; return (a + (p.year || "")).toLowerCase() + "_" + i; }
...
function regenerateProvenance(bodyMd, title, slug) {
  ...
  const bib = parsed.length
    ? parsed.map(function (p, i) {
        const L = ["@misc{" + refKey(p, i + 1) + ",", "  author = {" + bibEsc(p.authors) + "},", "  title = {" + bibEsc(p.title) + "},",
...
async function aiText(env, prompt, model, maxTokens) {
  const m = model || MODEL;
  // Reasoning models need headroom: reasoning_content is charged against max_tokens, so a
  // budget sized for prose alone comes back with EMPTY content and finish_reason=length.
  const res = await env.AI.run(m, { messages: [{ role: "user", content: prompt }], max_tokens: maxTokens || 8000 }, { gateway: { id: "default" } });
  let text = "";
  try {
    if (res && typeof res === "string") text = res;
    else if (res && typeof res.response === "string" && res.response) text = res.response;
    else if (res && typeof res.result === "string" && res.result) text = res.result;
    else if (res && res.choices && res.choices[0] && res.choices[0].message) text = String(res.choices[0].message.content || "");
    else text = "";
  } catch (e) { text = ""; }
  return text.trim();
}
...
function parseJsonObject(text) {
  ...
  // LaTeX backslashes (\zeta, \beta, \varepsilon) are not valid JSON escapes;
  // escape a backslash not followed by a JSON escape char (quote/backslash/slash).
  text = text.replace(/\\(?!["\\/])/g, "\\\\");
...
async function selectCandidates(env, limit) {
  ...
  const doneSet = new Set(((done && done.results) || []).map(function (r) { return r.slug; }));
  ...
  const pendSet = new Set(((pend && pend.results) || []).map(function (r) { return r.slug; }));
  const out = [];
  for (const p of all) {
...
async function verifySingleVersion(env, recId) {
  ...
  try {
    ...
    if (hits.length) { count = hits.length; latestDoi = hits[hits.length - 1].doi || rec.doi || null; }
  } catch (e) { /* keep count=1 */ }
...
function auditPrompt(paper) {
  return [
    ...
    (paper.body_md || "").slice(0, 11000)
  ].join("\n");
}

function normWs(s) { return String(s || "").replace(/\s+/g, " ").trim(); }

// Build normalized body + a map from normalized index -> original index so a
// normalized match can be mapped back to the original (line-wrapped) span.
function buildNormMap(s) {
  ...
  for (let i = 0; i < s.length; i++) {
    const c = s[i];
    if (/\s/.test(c)) {
      if (norm.length && norm[norm.length - 1] !== " ") { norm.push(" "); map.push(i); }
    } else { norm.push(c); map.push(i); }
  }
...
function applyEdits(md, issues) {
  ...
  for (const it of issues) {
    ...
    const fix = it.fix || "";
    if (!loc) { skipped.push({ reason: "empty location", category: it.category }); continue; }
    let start = -1, len = 0, method = null;
    // 1. exact match
    const ex = out.indexOf(loc);
    if (ex >= 0) { start = ex; len = loc.length; method = "exact"; }
    // 2. whitespace-normalized match (handles hard line-wraps in the body)
    if (start < 0) {
    ...
    if (start < 0) { skipped.push({ reason: "location not found", category: it.category, location: loc.slice(0, 80) }); continue; }
    out = out.slice(0, start) + fix + out.slice(start + len);
    applied.push({ category: it.category, method: method });
  }
...
function applyVersionMarkers(md, versionTo) {
  ...
  out = out.replace(/^(version:\s*["']?)[^"'\n]+(["']?)\s*$/im, function (m, p1, p2) { return p1 + versionTo + p2; });
  return out;
}
...
async function processPaper(env, paper, mode) {
  ...
  if (!recId) { return { slug: paper.slug, skipped: true, reason: "no zenodo recid", doi }; }

  const v = await verifySingleVersion(env, recId);
  if (v.count >= 2) {
    if (!dry) {
      await env.WATCH_DB.prepare("INSERT INTO paper_revision_log (slug, doi, title, version_from, status, audit_summary, created_at, updated_at) VALUES (?, ?, ?, ?, 'already-revised', ?, datetime('now'), datetime('now'))")
        .bind(paper.slug, doi, paper.title, paper.version, JSON.stringify({ zenodo_versions: v.count })).run();
    }
  ...
  try {
    ...
    if (parsed && Array.isArray(parsed.issues)) { findings = parsed; auditParsed = true; }
  } catch (e) { findings = { issues: [], auditError: e.message }; }
  const issues = findings.issues || [];
  const high = issues.filter(function (i) { return i.severity === "high"; });
  const low = issues.filter(function (i) { return i.severity !== "high"; });
  const auditSummary = { total: issues.length, low: low.length, high: high.length, categories: issues.map(function (i) { return i.category; }), parsed: auditParsed, rawLen: rawAuditLen };

  if (high.length) {
    if (!dry) {
      await env.WATCH_DB.prepare("INSERT INTO paper_revision_log (slug, doi, title, version_from, status, audit_summary, findings_json, created_at, updated_at) VALUES (?, ?, ?, ?, 'flagged', ?, ?, datetime('now'), datetime('now'))")
        .bind(paper.slug, doi, paper.title, paper.version, JSON.stringify(auditSummary), JSON.stringify(issues).slice(0, 4000)).run();
    }
  ...
  const appliedCats = edits.applied.map(function (a) { return a.category; }).join(", ") || "no substantive corrections required";
  ...
  await env.WATCH_DB.prepare("INSERT INTO version_queue (paper_doi, slug, title, version_from, version_to, corrected_md, status, references_bib, citation_audit, due_diligence, project_plan, readme_md, verify_script, verify_output, license_md, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 'drafted', ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))")
    .bind(doi, paper.slug, paper.title, paper.version, versionTo, revised, merged.references_bib, merged.citation_audit, merged.due_diligence, merged.project_plan, merged.readme_md, merged.verify_script, merged.verify_output, merged.license_md).run();
  await env.WATCH_DB.prepare("INSERT INTO paper_revision_log (slug, doi, title, version_from, version_to, status, audit_summary, changelog, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 'queued', ?, ?, datetime('now'), datetime('now'))")
    .bind(paper.slug, doi, paper.title, paper.version, versionTo, JSON.stringify(auditSummary), changelog).run();
...
async function runOnce(env, mode) {
  ...
  for (const p of candidates) {
    try { results.push(await processPaper(env, p, mode)); }
    catch (e) { results.push({ slug: p.slug, error: String(e && e.message || e).slice(0, 200) }); }
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/run/scan") { const mode = url.searchParams.get("mode") || "dry"; try { return json(await runOnce(env, mode)); } catch (e) { return json({ ok: false, error: e.message }, 500); } }
    if (url.pathname === "/debug/audit") {
      ...
      try {
        ...
        return json({ ok: true, slug, rawLen: t.length, raw: t.slice(0, 3000), parsed: parseJsonObject(t) });
    ...
    if (url.pathname === "/debug/ai") {
      ...
      try { const t = await aiText(env, p, m); return json({ ok: true, model: m, reply: t.slice(0, 500) }); } catch (e) { return json({ ok: false, model: m, error: String(e && e.message || e).slice(0, 300) }, 500); }
    }
    if (url.pathname === "/run/status") { try { return json(await statusSweep(env)); } catch (e) { return json({ ok: false, error: e.message }, 500); } }
    return json({ error: "not found" }, 404);
  },
  async scheduled(event, env, ctx) {
    try { const r = await runOnce(env, "live"); console.log("[qnfo-paper-reviser] cron done:", JSON.stringify({ candidates: r.candidates, results: r.results.map(function (x) { return { slug: x.slug, queued: x.queued, flagged: x.flagged, skipped: x.skipped, error: x.error }; }) })); }
    catch (e) { console.error("[qnfo-paper-reviser] cron error:", e.message); }
  }
};
