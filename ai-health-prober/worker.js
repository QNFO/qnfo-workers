...
async function probeOne(env, m) {
  ...
  let lastErr = null;
  for (let i = 0; i < bodies.length; i++) { try { const t0 = Date.now(); await env.AI.run(m.id, bodies[i]); return { ok: true, ms: Date.now() - t0 }; } catch (e) { lastErr = String((e && e.message) || e).slice(0, 120); } }
  return { ok: false, err: lastErr };
}
function toMs(m) {
  if (m == null) return null;
  if (typeof m === "number") return m > 1e12 ? m : m * 1000;
  var s = String(m).trim();
  if (/^[0-9]+$/.test(s)) { var n = Number(s); return n > 1e12 ? n : n * 1000; }
  var iso = s.replace(" ", "T");
...
async function runProbe(env) {
  ...
  for (let i = 0; i < MODELS.length; i++) {
    const m = MODELS[i]; const r = await probeOne(env, m); const mid = m.internal || m.id;
    results.push({ model: mid, ok: r.ok, ms: r.ms || null });
    if (env.QNFO_AUDIT) { try {
      if (r.ok) { await env.QNFO_AUDIT.prepare("UPDATE ai_model_health SET status=?5, last_probe_ts=?2, last_latency_ms=?3, consecutive_failures=0, updated_at=?4 WHERE model_id=?1").bind(mid, now, r.ms || null, new Date(now).toISOString(), "ok").run(); }
      else { await env.QNFO_AUDIT.prepare("UPDATE ai_model_health SET status=CASE WHEN consecutive_failures >= 2 THEN ?6 ELSE ?5 END, last_probe_ts=?2, consecutive_failures=COALESCE(consecutive_failures,0)+1, updated_at=?4 WHERE model_id=?1").bind(mid, now, null, new Date(now).toISOString(), "degraded", "failing").run(); }
    } catch (e) {} }
  }
  let up = 0; for (let i = 0; i < results.length; i++) { if (results[i].ok) up++; }
  return { probed: results.length, up: up, down: results.length - up };
}
async function checkFreshness(env) {
  ...
  for (let i = 0; i < SIGNALS.length; i++) {
    ...
    try {
      ...
      const ms = toMs(maxTs);
      if (ms != null) { ageH = Math.round((now - ms) / 360000) / 10; if (ageH > s[3]) status = (mode === "event") ? "idle" : "stale"; else status = "fresh"; }
      else { status = "no-data"; }
    } catch (e) { status = "error"; }
    out.push({ name: s[0], table: s[1], ts: s[2], mode: mode, max_ts: maxTs == null ? null : String(maxTs), age_hours: ageH, threshold_hours: s[3], status: status });
    try {
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (u.pathname === "/run") { const p = await runProbe(env); const f = await checkFreshness(env); let st = 0, idle = 0; for (let i = 0; i < f.length; i++) { if (f[i].status === "stale") st++; if (f[i].status === "idle") idle++; } return json({ ok: true, version: VERSION, probe: p, freshness: f, stale_count: st, idle_count: idle }); }
