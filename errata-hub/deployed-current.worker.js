...
var erratarespondMod = (function(){
const QNFO_VERSION = "qnfo-errata-respond/fabric-20260910";
var __defProp = Object.defineProperty;
var __name = (target, value) => __defProp(target, "name", { value, configurable: true });
...
var erratapublishMod = (function(){
const QNFO_VERSION = "qnfo-errata-publish/fabric-20260910";
var __defProp = Object.defineProperty;
var __name = (target, value) => __defProp(target, "name", { value, configurable: true });
...
var PuppeteerWorkers = class extends Puppeteer {
  ...
  async connect(endpoint, sessionId, options) {
    ...
    try {
      ...
      const connectionTransport = await WorkersWebSocketTransport.create(endpoint, sessionId, { browser, guardrails: options?.guardrails });
...
function markdownToHtml(md) {
  ...
  const head = "<!DOCTYPE html><html><head><meta charset='utf-8'><title>Paper</title><script src='https://cdn.jsdelivr.net/npm/mathjax@3/es5/tex-chtml.js' async><\/script><style>body{max-width:40em;margin:2em auto;font-family:Georgia,serif;line-height:1.6;padding:0 1em}</style></head><body>";
...
async function notifyUser(env, action, paper, pub, err) {
  ...
  try {
    ...
    const lines = err ? ["The auto-publish of an errata correction failed.", "", "Paper: " + (paper && paper.slug || action && action.paper_doi || ""), "Error: " + err] : ["An errata correction was published automatically (cloud pipeline).", "", "Paper: " + (paper && paper.slug || ""), "New DOI: " + (pub.newDoi || ""), "Version: " + (pub.version || ""), "PDF: " + (pub.pdf ? "regenerated in-worker" : "failed (" + (pub.pdfError || "n/a") + ")"), "", "This is an automatic receipt. A post-publish adversarial audit is recommended."];
...
async function runPublish(env, mode) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      results.push(await publishAction(env, a));
    } catch (e) {
...
var publish_worker_src_default = {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/run/publish") {
      const mode = url.searchParams.get("mode") || "dry";
      try {
        return json(await runPublish(env, mode));
      } catch (e) {
        return json({ ok: false, error: e.message }, 500);
      }
    }
...
return { default: publish_worker_src_default };
})();
...
export default {
  ...
  async scheduled(event, env, ctx) {
    ...
    if (c === "15 * * * *") return erratarespondMod.default.scheduled(event, env, ctx);
    if (c === "30 * * * *") return erratapublishMod.default.scheduled(event, env, ctx);
  },
};