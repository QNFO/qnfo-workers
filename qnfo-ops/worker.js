...
var DEFAULT_MAX_OUT = 131072; // OUT-128K-1 (2026-09-09): maximize output to 128K - client ops-exec maxOutput=131072, upstream DeepSeek accepts 384000; server was clamping the client 128K down to 64K (truncation fix)
...
async function handleRelay(env, body, messages, maxTokens, isStream, ua, ctx) {
  ...
  const maxOut = clamp(maxTokens, 131072); // OUT-128K-1: no 8192/32K ops clamp on the relay path
...
async function handleChat(env, body, authHeader, ua, ctx) {
  ...
  const answerCap = clamp((Number.isFinite(max_tokens) && max_tokens > 0) ? max_tokens : DEFAULT_MAX_OUT, Math.min(DEFAULT_MAX_OUT, envInt(env, "OPS_ANSWER_CAP", 131072)));
...
export class OpsExecWorkflow extends WorkflowEntrypoint {
  async run(event, step) {
    ...
    const answerCap = clamp(Number.isFinite(body.max_tokens) && body.max_tokens > 0 ? body.max_tokens : DEFAULT_MAX_OUT, Math.min(DEFAULT_MAX_OUT, envInt(env, "OPS_ANSWER_CAP", 131072)));
    const temperature = (body && typeof body.temperature === "number" && body.temperature >= 0 && body.temperature <= 2) ? body.temperature : envFloat(env, "OPS_TEMPERATURE", 0.5);
    const topP = (body && typeof body.top_p === "number" && body.top_p > 0 && body.top_p <= 1) ? body.top_p : envFloat(env, "OPS_TOP_P", 0.9);
    const toolResultCap = envInt(env, "OPS_TOOL_RESULT_CAP", 32768);
