...
function auth(req, env) {
  ...
  if (!tok) return false;
  const check = /* @__PURE__ */ __name((exp) => {
...
async function postText(s, text, reply) {
  const record = { text: truncate(text, 290), createdAt: (/* @__PURE__ */ new Date()).toISOString() };
...
function sanitizePosts(raw) {
  return (raw || []).map(function(x) {
    return truncate(String(x), 290);
  }).filter(function(x) {
    return x.trim();
  });
}
__name(sanitizePosts, "sanitizePosts");
...
async function checkThread(env, title, abstract, posts) {
  ...
  function parseIssues(text2) {
    if (!text2) return null;
    const cleaned = String(text2).replace(/```(?:json)?/g, "").trim();
    try {
      const parsed = JSON.parse(cleaned);
      if (Array.isArray(parsed)) return parsed.filter(function(x) {
        return x && x.issue;
      });
    } catch (e) {
    }
  ...
  const ai1 = await env.AI.run(COMPOSE_MODEL, { messages: [{ role: "user", content: base }], max_tokens: 3e3 });
...
__name(checkThread, "checkThread");
...
async function alertDigest(env) {
  try {
    ...
    const summary = "QNFO alerts digest " + (/* @__PURE__ */ new Date()).toISOString() + " - " + items.length + " alert(s): " + items.map(function(a) {
      return "[" + a.level + "] " + a.source + ": " + a.message;
    }).join(" | ");
...
__name(alertDigest, "alertDigest");
async function autoScan(env) {
  try {
    ...
    if (!r.ok) {
      console.error("auto-scan zenodo fetch failed", r.status);
      return;
    }
    ...
    for (const h of hits) {
      ...
      const abstract = String(md.description || "").replace(/<[^>]+>/g, "").slice(0, 4e3);
      ...
      const ai = await env.AI.run(COMPOSE_MODEL, { messages: [{ role: "user", content: prompt }], max_tokens: 2e3 });
...
async function bufferPost(env, text) {
  ...
  try {
    ...
    for (const svc of ["mastodon", "linkedin", "twitter"]) {
      ...
      if (!ch) {
        results.push({ platform: svc, status: "no-channel" });
        continue;
      }
      try {
...
async function drainQueue(env) {
  ...
  for (let i = 0; i < DRAIN_PER_RUN; i++) {
    ...
    try {
      ...
      try {
        ...
        bufferResult = { error: String(e && e.message || e) };
      }
  ...
  return { posted, failed, posted_today: postedToday + posted };
}
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (p === "/thread" && m === "POST") {
        ...
        const s = await session(env);
        const uris = await postThread(s, posts);
        return new Response(JSON.stringify({ ok: true, root: uris[0], count: uris.length, uris }), { headers: { "Content-Type": "application/json", ...cors } });
      }
      if (p === "/threads" && m === "GET") {
      ...
      if (p === "/compose" && m === "POST") {
        ...
        const abstract = String(b.abstract || "").slice(0, 4e3);
