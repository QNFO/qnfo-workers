...
TID_OPS="guard-probe-ops-$(date +%s%N | head -c 12)"
TID_IDEA="guard-probe-idea-$(date +%s%N | head -c 12)"
...
echo "  SELECT thread_id, ts, status FROM intent_express_log WHERE thread_id LIKE \"guard-probe-%\";"
echo "Expected: only the idea thread row, status http:201."