// qnfo-ddocs-indexer v1.0.1 - D-docs indexer to qnfo-ddocs R2 + Vectorize.
// v1.0.1 (PATCH-2026-09-13-prefix-resolver, qnfo-ops):
//  Defect fixed: processObject() and handleIndex() defaulted the R2 prefix to the literal
//  "2026-09-04" (the d-drive snapshot date). Once content stopped landing under that one date
//  folder the cron indexed nothing, so ddocs_index_state.indexed_at froze at 2026-09-04 while the
//  worker kept firing (13 req24) - a firing-no-output worker.
//  Fix: DDOC_PREFIX env still wins. Otherwise the newest YYYY-MM-DD/ top-level prefix present in
//  the bucket is auto-detected. If no dated prefix exists the prefix filter is dropped entirely,
//  so the indexer tracks whatever content is actually there. The effective prefix is now returned
//  by /index and /stats so the next stall is visible instead of silent.
//  VERSION is now a strict-semver constant exposed on /health (was the inline "v1.0-server-side").
var VERSION = "1.0.1";
...
var SKIP_FRAG = ["node_modules", "/.git/", ".wrangler/", "/dist/", "/build/", "/.obsidian/workspace", "desktop.ini", "_manifests/"];
...
function chunkText(text) {
  ...
  while (start < n) {
    ...
    if (end < n) {
      ...
      if (p > start + CHUNK_SIZE / 2) end = p + 1;
    }
...
// Resolve the R2 prefix to index. DDOC_PREFIX wins; otherwise use the newest dated top-level
// prefix in the bucket; otherwise no prefix filter at all. Never a hardcoded date.
...
var worker_default = {
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      return json({ error: "EXCEPTION", detail: e.message }, 500);
    }
  }
};
export {
