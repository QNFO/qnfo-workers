...
**Every sweep kills jobs seconds-to-minutes old.** No 20-minute or 45-minute staleness threshold can produce a 27-second-old victim. So the sweeps are **not** age-threshold reapers; they reconcile over "all non-terminal rows at sweep time," which cannot distinguish a live generation from an abandoned one. This also **falsifies the "~20 minute reaper" hypothesis** recorded for the 13:51 sweep — its newest victim was 2.1 min old.

Two writers, two timestamp formats (`2026-09-13T14:41:28Z` vs `2026-09-13 14:35:31`), neither matching `new Date().toISOString()` as used by `qnfo-ops-jobs-reaper/worker.js`. **That worker is not deployed** — it is absent from the 55-worker fleet, and its own `FINDING-reaper-is-redundant.md` says do not deploy it. The deployed sweepers remain **unidentified**.
...
**Issues 463 and 464 already cover these two signatures — both closed `wontfix`.** The signature then recurred twice *after* that closure (09-11 and 09-12, both `mobile`). Row 463 has been annotated with this recurrence.
...
2. **Identify the two deployed sweepers** (two timestamp formats, neither the repo reaper's). Until they are identified, no threshold change is safe.
3. **`terminal_at`** so sweep-assigned terminals are distinguishable from real ones.
4. **F14**: persist real `reasoning_content` (or stop implying thinking mode on replay). Currently the payload stores only a placeholder, so "pass it back" is not implementable as-is.
5. **Reopen 463/464** or record them as recurring-but-wontfix knowingly.
