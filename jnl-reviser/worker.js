// jnl-reviser v0.1.2 — referee-driven revision loop for self-owned QNFO papers.
// Reads referee weaknesses -> Workers AI proposes targeted edits -> applies to staged
// new-version draft (full-metadata merge PUT) -> optional publish.
// Deployed: https://jnl-reviser.q08.workers.dev
// Stack: KV jnl-state, AI binding, service binding REF -> jnl-referee
// Secrets: ZENODO_TOKEN, JNL_OPS_TOKEN (x-ops-token header)
const REF = "jnl-referee";
...
export default {
  async fetch(request, env){
    ...
    try {
      if(path==="/health") return json({ok:true,service:"jnl-reviser",version:"0.1.2",model:MODEL,zenodo_token_set:!!env.ZENODO_TOKEN});
      ...
      if(applied.length===0){
        ...
        if(applied.length===0) return json({ok:false,error:"no safe edits applied",applied,skipped,weaknesses:weak.slice(0,6),model_raw:raw.slice(0,800),text_chars:mdText.length,text_preview_chars:mdLimited.length},422);
      }
      ...
      const bump = (v)=>{ const m=String(v).match(/^v?(\d+)\.(\d+)(.*)$/i); if(!m) return `v1.1`; const nv = `${m[1]}.${Number(m[2])+1}`; return m[3]? `v${nv}` : /^v/i.test(v)? `v${nv}` : nv; };
      ...
      // full-metadata merge PUT (deposit PUT is full-replace)
