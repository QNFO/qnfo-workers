...
function normalizeResponsesInput(body) {
  ...
  if (Array.isArray(input)) {
    // RESPONSES-TOOL-ID-1 (2026-09-05): the ai-sdk Responses client echoes assistant function_call
    // items with id but NOT call_id (the Responses item id IS the call identity). Resolve id =
    // call_id || id; previously normalizeResponsesInput minted a RANDOM tool_call id that no longer
    // matched the following function_call_output -> DeepSeek 400 "assistant message with tool_calls
    // must be followed by tool messages responding to each tool_call_id" -> client saw "Request
    // failed / Bad Gateway" (canonical: 855KB ua=node /v1/responses history, 3x at 20:35; repro
    // Body E fc{id no call_id} + fco{call_id}). Defer a function_call that has neither id nor
    // call_id and pair it with the next function_call_output call_id; also keep assistant message
    // items that carry a chat-style tool_calls array (previously dropped -> orphan tool messages).
    let pendingFc = null;
    const pushToolCall = function (fid, fname, fargs) {
      messages.push({ role: "assistant", content: "", tool_calls: [{ id: fid, type: "function", function: { name: fname || "", arguments: typeof fargs === "string" ? fargs : JSON.stringify(fargs || {}) } }] });
    };
    for (const item of input) {
      if (!item || typeof item !== "object") continue;
      if (item.type === "function_call") {
        const cid = item.call_id || item.id || "";
        if (cid) pushToolCall(cid, item.name, item.arguments);
        else pendingFc = item;
      } else if (item.type === "function_call_output") {
        let cid = item.call_id || item.id || "";
        if (pendingFc) {
          if (!cid) cid = "call_" + randId("");
          pushToolCall(cid, pendingFc.name, pendingFc.arguments);
          pendingFc = null;
        }
        if (!cid) cid = "call_" + randId("");
        ...
        if (pendingFc) { pushToolCall("call_" + randId(""), pendingFc.name, pendingFc.arguments); pendingFc = null; }
        ...
        if (role === "assistant" && Array.isArray(item.tool_calls) && item.tool_calls.length) {
          base.tool_calls = item.tool_calls.map(function (tc) {
            ...
            return { id: (tc && (tc.id || tc.call_id)) || ("call_" + randId("")), type: "function", function: { name: tc && (tc.name || tcf.name) ? (tc.name || tcf.name) : "", arguments: typeof tcf.arguments === "string" ? tcf.arguments : JSON.stringify((tc && tc.arguments) || {}) } };
          });
        }
        messages.push(base);
      }
    }
    if (pendingFc) pushToolCall("call_" + randId(""), pendingFc.name, pendingFc.arguments);
  }
  return messages;
}
