...
var INTAKE_STALL_MIN = 120;
var INTAKE_BACKLOG_ALERT_N = 5;
...
async function escIssue(env, title, desc, cat, prio) {
  try {
    ...
    return { inserted: false, error: String(e.message).slice(0, 120) };
  }
}
__name(escIssue, "escIssue");
async function ensureSchema(env) {
  try {
    await env.QNFO_AUDIT.prepare("ALTER TABLE research_queue ADD COLUMN recover_count INTEGER DEFAULT 0").run();
  } catch (e) {
  }
  try {
    await env.QNFO_AUDIT.prepare("ALTER TABLE research_queue ADD COLUMN terminal_rearms INTEGER DEFAULT 0").run();
  } catch (e) {
  }
}
...
async function health(env) {
  ...
  const m = {};
  for (const r of rows.results || []) m[r.status] = r.n;
  ...
  return { queued: m.queued || 0, researching: m.researching || 0, review: m.review || 0, failed: m.failed || 0, published: m.published || 0, intake_new, intake_oldest, triaged };
}
...
async function intakeWatchdog(env) {
  ...
  if (h.intake_oldest) {
    ...
    if (isFinite(ms)) oldestAgeMin = (Date.now() - ms) / 6e4;
  }
...
__name(intakeWatchdog, "intakeWatchdog");
...
__name(recoverStale, "recoverStale");
...
__name(recoverFailed, "recoverFailed");
async function rearmTerminal(env) {
  ...
  for (const t of rows.results || []) {
    ...
    if (up && up.meta && up.meta.changes) {
      n++;
      try {
        await env.QNFO_AUDIT.prepare("INSERT INTO alerts (source, level, message) VALUES (?,?,?)").bind(WORKER, "info", "research_queue terminal auto-rearmed id=" + t.id + " (" + String(t.source_id || "?").slice(0, 40) + ", " + Math.round(ageH * 10) / 10 + "h old) -> queued fresh cycle").run();
      } catch (e) {
      }
      try {
        await env.QNFO_AUDIT.prepare("UPDATE agent_issues SET status='closed', updated_at=datetime('now') WHERE status='open' AND title LIKE ?1").bind("%TERMINAL research failure " + String(t.source_id || "").slice(0, 40) + "%").run();
      } catch (e) {
      }
    }
  }
...
__name(rearmTerminal, "rearmTerminal");
...
__name(terminalFailures, "terminalFailures");
async function escalateTerminal(env, terminal) {
  for (const t of terminal) {
    ...
    try {
      await env.QNFO_AUDIT.prepare("INSERT INTO alerts (source, level, message) VALUES (?,?,?)").bind(WORKER, "critical", "terminal research failure " + String(t.source_id || t.id || "?").slice(0, 40) + " -> agent_issues " + (r.inserted ? "ok" : "dup") + ": " + String(t.error || "").slice(0, 200)).run();
    } catch (e) {
    }
  }
}
__name(escalateTerminal, "escalateTerminal");
...
__name(versionErrors, "versionErrors");
async function escalateVersion(env, rows) {
  for (const v of rows) {
    ...
    if (r.inserted) {
      try {
        await env.QNFO_AUDIT.prepare("INSERT INTO alerts (source, level, message) VALUES (?,?,?)").bind(WORKER, "warning", "version_queue error escalated id=" + v.id + " -> agent_issues " + r.id).run();
      } catch (e) {
      }
    }
  }
}
__name(escalateVersion, "escalateVersion");
  ...
  for (const v of rows) {
    ...
    if (ageH >= VQ_RETRY_HOURS && !tooOld) {
      ...
      if (up && up.meta && up.meta.changes) {
        n++;
        try {
          await env.QNFO_AUDIT.prepare("INSERT INTO alerts (source, level, message) VALUES (?,?,?)").bind(WORKER, "info", "version_queue auto-rearmed id=" + v.id + " (error " + Math.round(ageH * 10) / 10 + "h old) -> drafted for drain").run();
        } catch (e) {
        }
      }
    }
...
__name(rearmVersion, "rearmVersion");
async function digestAlerts(env) {
  try {
    await env.QNFO_AUDIT.prepare("UPDATE alerts SET digested='auto' WHERE source=?1 AND (digested IS NULL OR digested='') AND created_at < datetime('now','-30 minutes')").bind(WORKER).run();
  } catch (e) {
  }
}
__name(digestAlerts, "digestAlerts");
async function run(env) {
  ...
  if (h.failed > 0 || stalled > 0 || terminal.length > 0 || vErr.length > 0 || intakeEscalating) {
    ...
    try {
      await env.QNFO_AUDIT.prepare("INSERT INTO alerts (source, level, message) VALUES (?,?,?)").bind(WORKER, level, msg).run();
    } catch (e) {
    }
  }
  await digestAlerts(env);
  try {
    await env.QNFO_AUDIT.prepare("INSERT INTO cloud_ops_events (id, ts, kind, text, meta, job, status) VALUES (?,?,?,?,?,?,?)").bind("po-" + Date.now().toString(36) + "-" + Math.floor(Math.random() * 1e6).toString(36), nowIso(), "health", JSON.stringify(h), JSON.stringify({ intake }), WORKER, "ok").run();
  } catch (e) {
  }
  return Object.assign({ recovered, recoveredStale, recoveredFailed, rearmedTerminal, terminal: terminal.length, vqErr: vErr.length, rearmed, intake }, h);
}
__name(run, "run");
var worker_default = {
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/health") {
      const h = await health(env);
      return json({ ok: true, worker: WORKER, version: VERSION, pipeline: h });
    }
