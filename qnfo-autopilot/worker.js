...
async function thinkLoop(env) {
  ...
  try {
    ...
    if (q && q.question) {
      // THINK-LOOP-DEDUP (2026-09-15): the generator re-emits the same research theme hourly
      // (22 open questions / 10 distinct prefixes before this guard). Skip if a thematically
      // near-duplicate open question exists in the last 7 days. Root-cause fix: stop the source.
      const _sig = (s) => { const o = new Set(); for (const w of String(s || '').toLowerCase().replace(/[^a-z0-9 ]/g, ' ').split(/\s+/)) { if (w.length > 4) o.add(w); } return o; };
      const _contain = (a, b) => { if (!a.size || !b.size) return 0; let n = 0; for (const x of a) if (b.has(x)) n++; return n / Math.min(a.size, b.size); };
      ...
      const _dup = (_recent.results || []).some(function (r) { return _contain(_cand, _sig(r.question)) >= 0.6; });
