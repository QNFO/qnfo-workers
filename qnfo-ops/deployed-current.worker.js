      ...
      if (body.stream) {
        ...
        const stream = new ReadableStream({
          start(controller) {
            ...
            // STREAM-ITEM-EVENTS-1 (2026-09-05, F4 resolution): the Responses API streaming contract
            // reconstructs output from item-level SSE events (output_item.added -> *_delta -> *_done ->
            // output_item.done). Previously only output_text.delta + response.completed were emitted, so
            // a strict streaming client could not reconstruct function_call items (requires_action).
            // Emit the full item-level sequence with EMPTY added-item payloads (deltas carry text or
            // arguments once), then response.completed with the full respObj for direct readers - no
            // double-count on either path (clients key events by item_id per the contract).
